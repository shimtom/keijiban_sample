...
module.exports = function (connection) {
  ...
  router.post('/boards', function (req, res, next) {
    ...
    // username が存在するかを確認.
    let query = "SELECT name, display_name FROM users WHERE name = ?";
    ...
    connection.query(query, [title, username], function (error, results) {
      if (error) {
        res.status(500).json(sql_error(error.sqlMessage, "boards", error.code));
        return;
      }

      let board_id = results.insertId;
      ...
      connection.query(query, [board_id], function (error, results) {
        if (error) {
          res.status(500).json(sql_error(error.sqlMessage, "boards", error.code));
          return;
        }
  ...
  router.get('/boards/:board_id/comments', function (req, res, next) {
    ...
    // 指定されたboardの存在を確認.
    let query = "SELECT id FROM boards WHERE boards.id = ?";
    connection.query(query, [board_id], function (error, results) {
      if (error) {
  ...
  router.post('/boards/:board_id/comments', function (req, res, next) {
    ...
    // 指定されたusernameの存在を確認
    let query = "SELECT name FROM users WHERE users.name = ?";
    connection.query(query, [username], function (error, results) {
      if (error) {
        res.status(500).json(sql_error(error.sqlMessage, "boards/" + board_id + "/comments", error.code));
        return;
    ...
    connection.query(query, [board_id, content, username], function (error, results) {
      if (error) {
        res.status(500).json(sql_error(error.sqlMessage, "boards/" + board_id + "/comments", error.code));
        return;
      }
      ...
      connection.query(query, [comment_id], function (error, results) {
        if (error) {
          res.status(500).json(sql_error(error.sqlMessage, "boards/" + board_id + "/comments", error.code));
          return;
        }
